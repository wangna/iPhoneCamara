...
@interface GDaiLiPingLunController()
{
...
@end
@implementation GDaiLiPingLunController
...
- (id)init
{
    self = [super init];
    if (self) {
    self.title = NSLocalizedString(@"专利评论",nil);
    }
    return self;
}
-(void)releaseSoap
...
//申请专利评论
#pragma mark - View lifecycle
-(IBAction)GPostSearchAgency:(NSString *)theStart
{
	[self releaseSoap];
	...
	NSDictionary *JsonDIc = [[NSDictionary alloc]initWithObjectsAndKeys:[self.gContentDic objectForKey:@"pnm"],@"pnm",[self.gContentDic objectForKey:@"an"],@"an",theStart,@"from",theToNum,@"to",LINK_PASSWORD,@"password",@"getCommentOfPatent",@"a",@"interface",@"m", nil];
	NSString *tString2 = [NSString stringWithFormat:@"%@%@%@",BASE_URL,@"&a=getCommentOfPatent&sid=",kkSessionID];
	NSString *tString3 = @"";
...
-(IBAction)GPostPingLunAgency:(NSString *)score AndInfo:(NSString *)content
{
	...
	NSDictionary *JsonDIc = [[NSDictionary alloc]initWithObjectsAndKeys:[self.gContentDic objectForKey:@"pnm"],@"pnm",[self.gContentDic objectForKey:@"an"],@"an",content,@"content",score,@"score",LINK_PASSWORD,@"password",@"gradePatent",@"a",@"interface",@"m", nil];
	NSString *tString2 = [NSString stringWithFormat:@"%@%@%@",BASE_URL,@"&a=gradePatent&sid=",kkSessionID];	
	NSString *tString3 = @"";
	[mSoapHttp PostSoadMessage:JsonDIc :tString2 :tString3];
	[JsonDIc release];
[MBProgressHUD showHUDAddedTo:self.view animated:YES];
...
- (void)viewDidLoad
{
    ...
    UIBarButtonItem *rightBtn=[[UIBarButtonItem alloc]initWithTitle:NSLocalizedString(@"发送",nil) style:UIBarButtonItemStyleBordered target:self action:@selector(soap)];
	...
	if (mSoapHttp.mArray.count>1) {
		...
		if ([[[mSoapHttp.mArray objectAtIndex:0]objectForKey:@"message"]length]==0) {
			if ([[[mSoapHttp.mArray objectAtIndex:0]objectForKey:@"status"]intValue]==0) {
				aString = NSLocalizedString(@"专利评论成功",nil);
			}else
				aString = NSLocalizedString(@"专利评论失败",nil);
		}else
...
{
	gAFooter = [[[PingLunFootView alloc]initWithFrame:CGRectMake(0, 0, 320, 140)]autorelease];
...
{
	self.mGtableView.center = CGPointMake(160, 208);
